...
const TALLY_URL = "https://tally.so/embed/mJbKed?transparentBackground=1";
  ...
  return (
    <GradientBackground>
      <SafeAreaView style={{ flex: 1 }} edges={["top", "bottom"]}>
        ...
        <Text style={styles.subtitle}>Be the first to get access to NXTATH.</Text>

        <View style={styles.webWrap}>
          {Platform.OS === "web" ? (
            <iframe
              key={reloadKey}
              title="NXTATH Waitlist"
              src={TALLY_URL}
          ...
          {failed && (
            <View style={styles.overlayCenter}>
              <View style={styles.errorCard}>
                ...
                <Pressable
                  accessibilityLabel="Retry loading waitlist form"
                  onPress={() => {
                    setReloadKey((k) => k + 1);
                    setFailed(false);
                    setLoading(true);
